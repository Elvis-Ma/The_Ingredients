# -*- coding: utf-8 -*-
"""
Created on Fri Jul 16 17:28:27 2021
...
def rec(i):
    ...
    if user_rating.empty:
        ...
        list1 = np.random.randint(20,60, food_recommemdation.shape[0])
        list_score=list1.tolist()
        list_score.sort(reverse=True)
        # print(list_score)
        food_recommemdation['Score'] = list_score
        print(food_recommemdation)
        ...
        food_recommemdation.rename(columns = {'Ingredient_title':'lacked_Ingredient_title'}, inplace = True)
        print(food_recommemdation)
        recommendation_Score=pd.DataFrame(recommendation_table_df, columns=['Score'])
        print(recommendation_Score)
        recommendation_Score=recommendation_Score.reset_index()
        print(recommendation_Score)
        recommendation_Score_Merge = pd.merge(food_recommemdation,recommendation_Score,on="Dishes_id")
        print(recommendation_Score_Merge)
        recommendation_Score_Merge['Score']=100*recommendation_Score_Merge['Score']
        print(recommendation_Score_Merge)
        cursor.execute("DELETE FROM member_recommendation WHERE member_id=\"{}\"".format(i))
        conn.commit()
    
      
        recommendation_Score_Merge.to_sql('member_recommendation', con=co, if_exists='append',index=False)    
