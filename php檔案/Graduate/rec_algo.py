...
def rec(i):
    ...
    command = "SELECT Dishes_id,title ,CONCAT(dishes_type,'.',food_main,'.',country_cusine,'.',M_or_V)AS 'genres' FROM `dishes`"
    ...
    command2="SELECT dishes_ingredient.dishes_id,group_concat(Ingredient_title) as Ingredient_title FROM (dishes_ingredient LEFT JOIN dishes ON dishes.Dishes_id=dishes_ingredient.Dishes_id)LEFT JOIN ingredient on dishes_ingredient.Ingredient_ID=ingredient.Ingredient_ID GROUP BY Dishes_id"
    ...
    command4 = "SELECT fridge_contain.FridgeID,fridge_link.member_ID,Ingredient.Ingredient_title,Ingredient_date FROM (`fridge_contain` LEFT JOIN ingredient ON fridge_contain.Ingredient_ID=ingredient.Ingredient_ID)RIGHT JOIN fridge_link ON fridge_link.FridgeID=fridge_contain.FridgeID WHERE fridge_link.member_id=\"{}\"".format(i)
