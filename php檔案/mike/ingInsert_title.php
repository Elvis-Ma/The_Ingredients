...
$one = $_REQUEST['ingredient_ID']; // 接受android端傳來的值
$two = $_REQUEST['FridgeID'];// 接受android端傳來的值
$three = $_REQUEST['expiration_date'];// 接受android端傳來的值

$qry_Select_title="SELECT `ingredient_ID` FROM ingredient WHERE `ingredient_title` = $one";
$result = mysqli_query($conn, $qry_Select_title); // 上方的執行結果丟給result變數
