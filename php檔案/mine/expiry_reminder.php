...
if (mysqli_affected_rows($conn)>0){
	$qry_Check="SELECT fridge_contain.FridgeID, ingredient.ingredient_title, fridge_contain.expiration_date FROM ingredient, fridge_contain WHERE fridge_contain.expiration_date <= DATE(NOW()) + INTERVAL + 7 DAY AND fridge_contain.expiration_date >= DATE(NOW()) AND fridge_contain.FridgeID = '$one' AND fridge_contain.Ingredient_ID = ingredient.ingredient_ID" ;
